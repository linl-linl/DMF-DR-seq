...
url = '/home/songjia/bigdisk/linl/xuxing_multiomics/RNA/expr_pattern2/test_3same_ref/script/'
url2 = '/home/songjia/bigdisk/linl/xuxing_multiomics/RNA/expr_pattern2/test_3same_ref/'
segcopy3 = args[1]
file_bounds = args[2]

final=read.table(segcopy3,header=T)
# final=na.omit(final)
...
for(k in 1:w){
  ...
  pdf(file=paste(url2, colnames(final)[k], "_CN.pdf", sep=""), width=75, height=30)

  top=3
  rectangles1=data.frame(pos[seq(1,nrow(pos), 2),])
  rectangles2=data.frame(pos[seq(2,nrow(pos), 2),])
  amp=data.frame(x=which(final[,k]>1.5), y=final[which(final[,k]>1.5),k])
  del=data.frame(x=which(final[,k]<0.5), y=final[which(final[,k]<0.5),k])
  flat=data.frame(x=which(final[,k]>=0.5 & final[,k]<=1.5), y=final[which(final[,k]>=0.5 & final[,k]<=1.5),k])
